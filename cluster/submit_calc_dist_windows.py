...
def submit_calc_dist_windows(number_of_windows_per_job, number_of_windows_per_class_path, mac_min_range, mac_max_range, mac_delta, maf_min_range, maf_max_range, maf_delta, job_stderr_folder, job_stdout_folder):
    ...
    if mac_min_range>0:
        for mac in range(mac_min_range, mac_max_range+1, mac_delta):
            ...
            while window_id <= num_windows:
                ...
                job_stdout_file=f'{job_stdout_folder}mac{mac}_windows{min_window_id}-{max_window_id}.stderr'
    ...
    if maf_min_range>0:
        for maf_int in range(maf_min_range, maf_max_range+1, maf_delta):
            ...
            while window_id <= num_windows:
                ...
                job_stdout_file=f'{job_stdout_folder}maf{maf}_windows{min_window_id}-{max_window_id}.stderr'
...
if __name__ == '__main__':
    ...
    number_of_windows_per_job = sys.argv[10]
    ...
    submit_calc_dist_windows(number_of_windows_per_job, number_of_windows_per_class_path, mac_min_range, mac_max_range, mac_delta, maf_min_range, maf_max_range, maf_delta, job_stderr_folder, job_stdout_folder)
