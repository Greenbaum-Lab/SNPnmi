...
def window_calc_pairwise_similarities_with_guardrails(window_df, min_valid_sites_precentage, min_minor_freq_expected,
    ...
    with Timer("Actual function"):
        ...
        for site_index in range(len(window_df.columns)):
            ...
            site_calc_pairwise_similarity_with_guardrails(window_df, site_index, min_valid_sites_precentage,
                                                          min_minor_freq_expected, max_minor_freq_expected,
                                                          min_minor_count_expected, max_minor_count_expected,
                                                          window_pairwise_counts, window_pairwise_dist)
    return window_pairwise_counts, window_pairwise_dist


def site_calc_pairwise_similarity_with_guardrails(window_df, site_index, min_valid_sites_precentage,
                                                  min_minor_freq_expected, max_minor_freq_expected,
                                                  min_minor_count_expected, max_minor_count_expected,
                                                  window_pairwise_counts, window_pairwise_similarity):
...
def calc_similarity_in_windows(dataset_name, mac_maf, class_value, min_window_index, max_window_index,
    ...
    for window_id in range(min_window_index, max_window_index + 1):
        ...
        window_pairwise_counts, window_pairwise_similarity = window_calc_pairwise_similarities_with_guardrails(
            window_df, min_valid_sites_precentage, min_minor_freq_expected, max_minor_freq_expected,
            min_minor_count_expected, max_minor_count_expected)
