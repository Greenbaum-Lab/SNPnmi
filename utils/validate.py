...
def validate_split_transposed_windows(mac_maf, class_name, min_window_id, max_window_id,
                                      expected_number_of_splits_per_window):
    ...
    for window_id in range(min_window_id, max_window_id + 1):
        ...
        with open(output_indexes_file, 'rb') as jsf:
            ...
            for split_id in split_id_2_indexes.keys():
                ...
                assert get_num_lines_in_file(
                    output_splits_transposed_template.format(split_id=str(split_id))) == expected_num_sites_in_slice
...
def _validate_count_dist_file(options, count_dist_file):
    try:
        ...
        if _file_len(count_dist_file) == get_num_individuals(options.dataset_name) - 1:
...
def validate_all_count_dist_files(mac_maf, class_name):
    ...
    assert os.path.isfile(
        f'{paths_helper.dist_folder}{mac_maf}_{class_name}_all_norm_dist.valid.flag'), f'{paths_helper.dist_folder}{mac_maf}_{class_name}_all_norm_dist.valid.flag'
    assert os.path.isfile(
        f'{paths_helper.dist_folder}{mac_maf}_{class_name}_all_count_dist.valid.flag'), f'{paths_helper.dist_folder}{mac_maf}_{class_name}_all_count_dist.valid.flag'
