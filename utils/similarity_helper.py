...
# TODO rename - similiarty
def calc_similarity_based_on_files(files):
...
def generate_similarity_matrix(windows_files, output_folder, output_files_name, override=False):
    ...
    all_count_similarities_file = f'{output_folder}{output_files_name}_count_similarity.tsv.gz'
    all_norm_similarities_file = f'{output_folder}{output_files_name}_norm_similarity.tsv.gz'
    if (not override) and os.path.isfile(all_count_similarities_file):
        print(f'all_count_distances_file exist, do not calc! {all_count_similarities_file}')
        return
    ...
    with Timer("Actual job"):
        dists, counts = calc_similarity_based_on_files(windows_files)

    # write (and validate) output
    write_pairwise_similarity(all_count_similarities_file, counts, dists)
    print(f'all_count_distances_file : {all_count_similarities_file}')
    _validate_count_dist_file(all_count_similarities_file)
