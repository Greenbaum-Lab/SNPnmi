...
def submit_netstcut(job_type, job_long_name, job_name, similarity_matrix_path, output_folder):
   ...
   # build netstrcut cmd
    netstruct_cmd = build_netstruct_cmd(similarity_matrix_path, output_folder)
    if netstruct_cmd:
...
def build_netstruct_cmd(similarity_matrix_path, output_folder):
    # validate the input
    ...
    return f'java -jar {jar_path} -ss 0.001 -minb 5 -mino 5 -pro {output_folder} -pm {similarity_matrix_path} -pmn {indlist_path} -pss {sample_sites_path} -w true'
