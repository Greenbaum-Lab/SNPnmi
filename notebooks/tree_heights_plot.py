...
def csv_to_plot(csv_path, plot_path, title):
    for num_of_snp in [1000, 5000]:
        ...
        for mac_maf in ['mac', 'maf']:
            ...
            for score in SCORES:
                ...
                avg = np.array(avg)
                std = np.array(std)
                plt.plot(class_names, avg, color=SCORE2COLOR_DICT[score], label=score, linewidth=line_width)
                ...
                if score == 'num_of_leaves':
                    plt.plot([min(class_names), max(class_names)],
                             ...
                             linestyle='--', color=SCORE2COLOR_DICT['num_of_leaves'], alpha=0.8, linewidth=line_width)
                if score == 'num_of_nodes':
                    plt.plot([min(class_names), max(class_names)],
                             [gt_nodes[options.dataset_name][0] for _ in range(2)],
                             linestyle='--', color=SCORE2COLOR_DICT['num_of_nodes'], alpha=0.8, linewidth=line_width)
